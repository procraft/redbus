...
Dashboard помимо счётчиков показывает плиточный обзор всех topics
([`src/components/TopicOverview.tsx`](src/components/TopicOverview.tsx)). Он строится из одного снимка
`/topic/stat` плюс `/repeat/stat`: consumers, суммарный lag, средний rate и время последнего сообщения
...
использует query-параметр как начальный фильтр и раскрывает выбранный topic.
...
Класс вычисляет `model.ErrorClass` ([`internal/app/model/errorclass.go`](../../internal/app/model/errorclass.go)):
изменчивые значения — числа, UUID, hex-хэши, timestamps и durations — заменяются плейсхолдерами (`<N>`,
`<UUID>`, `<HEX>`, `<TIME>`, `<DURATION>`), поэтому `task 58964360 failed` и `task 58964527 failed` дают один
класс `task <N> failed`. Цифры, приклеенные к букве (`int64`, `sha256`, `v2`), считаются частью имени. Поле
`error` содержит класс, `sample` — точный текст самого свежего падения класса. В PostgreSQL по-прежнему хранится
точный текст; SQL группирует по нему, а схлопывание по классам выполняется в Go. `/repeat/repeatError` перезапускает только сообщения выбранного класса за заданный период,
...
`/repeat/deleteError` безвозвратно удаляет только окончательно упавшие записи выбранного класса
(`finished_at IS NOT NULL`); активные и ожидающие следующей попытки повторы он не затрагивает. Оба действия
принимают в `error` класс или любой точный текст из него: repository сначала читает различающиеся тексты
упавших записей topic/group, отбирает принадлежащие классу и меняет только их (`error = ANY(...)`).
...
API списка отдельных failed messages отсутствует. React UI показывает агрегированный по классу
ошибки триаж с примером последнего точного текста, поддерживает restart как всего topic/group, так и одного класса ошибки, а также удаление
окончательно упавших сообщений класса после отдельного предупреждения о невозможности восстановления.
